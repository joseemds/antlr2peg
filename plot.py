...
def classify(entry):
    if "tracker" in entry:
        return "SUCCESS"
    kind = entry.get("kind", "")
    if kind in ("MISSING_FILE", "MULTIPLE_FILES", "LEFT_RECURSION", "MISSING_RULE", "HAS_SEMANTIC_ACTION", "WRONG_START_RULE"):
        return kind
    return "UNKNOWN"
...
order  = ["SUCCESS", "MISSING_FILE", "MULTIPLE_FILES", "LEFT_RECURSION", "HAS_SEMANTIC_ACTION" ,"MISSING_RULE", "WRONG_START_RULE", "UNKNOWN"]
